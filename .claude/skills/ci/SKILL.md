...
If any changed files are under `flacted/` or `tests/`, run the following agents **in order**:
...
- **changelog** - update `CHANGELOG.md` with entries for the changes. After it completes, check if
  ...
  the relevant commit. **Only run when changes affect users**: files under `flacted/`, `tests/`,
...
For example, if a commit contains `flacted/commands/main.py`,
`tests/test_main_command.py`, and `CHANGELOG.md`, the component is determined by
`flacted/commands/main.py` and `tests/test_main_command.py` only.
...
- Changes span unrelated components (e.g. `flacted/media.py` and `.claude/agents/release.md`).
...
For Python files, strip the `flacted/` prefix and replace `/` with `.` (like module imports).

- Python file `flacted/media.py` → `media:`.
- Multiple files under `flacted/commands/` → `commands:`.
- Single command file `flacted/commands/admin.py` → `commands.admin:`.
