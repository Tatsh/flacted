---
name: python-expert
description: Expert-level Python author for non-trivial implementation work in the flacted project. Use when the task needs idiomatic, production-quality Python beyond a simple edit.
---

# Python Expert Agent

General-purpose expert-level Python coding agent for the flacted project.
...
- `` :py:mod:`flacted.string` `` for modules
- `` :py:func:`flacted.string.slugify` `` for functions
- `` :py:class:`flacted.typing.ProbeDict` `` for classes
- `` :py:meth:`MyClass.my_method` `` for methods
- Use `~` to shorten the displayed name:
  `` :py:func:`~flacted.string.slugify` `` renders as `slugify`
